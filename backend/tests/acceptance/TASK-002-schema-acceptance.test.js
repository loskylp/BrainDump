 ...
 * database after migrations have been applied. They operate through the
 * database public interface (pg_catalog introspection and DML) — no access to
 * implementation internals beyond the Sequelize instance required to connect.
 *
 * Requirements traced:
 ...
 * Run:
 *   cd backend
 *   POSTGRES_URL=postgresql://braindump_dev:braindump_dev@localhost:5432/braindump_dev npm test
 */
...
describe('AC-2 [REQ-012]: UUID primary keys via gen_random_uuid()', () => {
  ...
  test.each(tables)('table "%s" has UUID PK with gen_random_uuid() default', async (tableName) => {
    ...
    // Then: the PK column is named "id", type "uuid", default gen_random_uuid()
    const [rows] = await sequelize.query(
...
describe('AC-3 [REQ-012]: Foreign key constraints with correct ON DELETE behavior', () => {
  ...
  test('[VERIFIER-ADDED] notes.folder_id delete rule is SET NULL, not CASCADE', async () => {
    // Given: the schema requires SET NULL so notes are not destroyed with their folder
  ...
  test('[VERIFIER-ADDED] CASCADE delete on user removes notes (end-to-end FK enforcement)', async () => {
    // Given: a user with a note exists in the database
    // When: the user row is deleted
    // Then: the note is also deleted — DB enforces ON DELETE CASCADE
...
describe('AC-4 [REQ-012]: TIMESTAMPTZ columns defaulting to NOW()', () => {
  ...
  test('[VERIFIER-ADDED] no plain TIMESTAMP (without time zone) columns on application tables', async () => {
    ...
    // Then: none exist — confirms no naive timestamp columns exist
    const [rows] = await sequelize.query(`
...
describe('AC-5 [REQ-011]: RLS enabled and forced on notes, folders, note_versions', () => {
  ...
  test.each(rlsTables)('RLS is FORCED on "%s"', async (tableName) => {
    const [rows] = await sequelize.query(
      `SELECT relforcerowsecurity FROM pg_class WHERE relname = :tableName`,
      { replacements: { tableName } }
    );
    expect(rows.length).toBe(1);
  ...
  test('[VERIFIER-ADDED] RLS policy on "notes" references current_setting app.current_user_id', async () => {
    ...
    // When: the policy expression is read
    // Then: it calls current_setting('app.current_user_id')
    const [rows] = await sequelize.query(
  ...
  test('[VERIFIER-ADDED] RLS is NOT enabled on the users table', async () => {
    // Given: only notes, folders, note_versions are RLS-protected per ADR-006
    // When: the users table rowsecurity flag is inspected
    // Then: it is false — users table is the identity store, not per-row isolated
    const [rows] = await sequelize.query(
      `SELECT rowsecurity FROM pg_tables
...
describe('AC-6 [REQ-011/REQ-012]: Migration role not blocked by RLS (OBS-002)', () => {
  test('all 5 application tables exist — DDL was not blocked by RLS', async () => {
    ...
    // Then: all 5 tables exist — DDL is not filtered by RLS policies
    const [rows] = await sequelize.query(
...
describe('AC-7 [REQ-011]: SET LOCAL app.current_user_id middleware', () => {
  const middlewarePath = path.join(__dirname, '../../src/middleware/rlsContext.js');

  test('rlsContext.js exists at src/middleware/rlsContext.js', () => {
  ...
  test('rlsContext.js contains SET LOCAL app.current_user_id', () => {
    // Given: the middleware must issue the SET LOCAL statement to configure RLS context
  ...
  test('rlsContext.js uses a null UUID fallback for unauthenticated requests', () => {
    // Given: unauthenticated requests must not match any user's rows via RLS
    // When: the source is inspected for the null UUID sentinel
    // Then: the null UUID 00000000-... is present — ensures RLS returns zero rows
    const src = fs.readFileSync(middlewarePath, 'utf8');
    expect(src).toContain('00000000-0000-0000-0000-000000000000');
  });

  test('[VERIFIER-ADDED] SET LOCAL scopes to transaction: value persists inside, resets after COMMIT', async () => {
    // Given: SET LOCAL must be wrapped in a transaction to guarantee isolation
    // When: SET LOCAL is issued inside BEGIN/COMMIT
    // Then: the value is visible within the transaction and gone after COMMIT
    //       This documents the behavioral constraint the middleware must satisfy.
...
describe('AC-9 [REQ-012]: notes_search_vector_update trigger fires on INSERT/UPDATE', () => {
  ...
  test('search_vector is populated on INSERT — positive end-to-end', async () => {
    // Given: a note with a title containing distinctive terms
    // When: the note is inserted into the RLS-protected table
    // Then: the trigger fires and search_vector is non-null
  ...
  test('search_vector updates on UPDATE of title — positive case', async () => {
    ...
    // Then: the search_vector contains the stemmed form of the new title
    ...
    expect(rows[0].sv).toContain('updat'); // PostgreSQL stems "Updated" -> "updat"
  ...
  test('[VERIFIER-ADDED] trigger fires on UPDATE of body column', async () => {
    // Given: the trigger targets title OR body columns
    // When: only the body is updated (title unchanged)
    // Then: the search_vector is updated to reflect the new body content
    const userId = 'ac9c0000-0000-0000-0000-000000000001';

    await sequelize.query(
      `INSERT INTO users (id, username, email, password_hash)
       VALUES (:id, 'ac9bodyuser', 'ac9-body@example.com', '$2a$12$fakehash')
       ON CONFLICT (id) DO NOTHING`,
      { replacements: { id: userId } }
    );
    ...
    await sequelize.query(
      `INSERT INTO notes (id, user_id, title, body)
       VALUES ('ac9c0000-0000-0000-0000-000000000002', :uid, 'Stable Title', 'oldcontent')`,
      { replacements: { uid: userId } }
    );

    await sequelize.query('SET LOCAL app.current_user_id = :id', { replacements: { id: userId } });
    await sequelize.query(
      `UPDATE notes SET body = 'newbodycontent' WHERE id = 'ac9c0000-0000-0000-0000-000000000002'`
    );

    await sequelize.query('SET LOCAL app.current_user_id = :id', { replacements: { id: userId } });
    const [rows] = await sequelize.query(
      `SELECT search_vector::text AS sv FROM notes WHERE id = 'ac9c0000-0000-0000-0000-000000000002'`
    );
    expect(rows[0].sv).toContain('newbodycont'); // stemmed from "newbodycontent"

    // Cleanup
    await sequelize.query('SET LOCAL app.current_user_id = :id', { replacements: { id: userId } });
    await sequelize.query(`DELETE FROM notes WHERE id = 'ac9c0000-0000-0000-0000-000000000002'`);
    await sequelize.query('DELETE FROM users WHERE id = :id', { replacements: { id: userId } });
  });
});
...
describe('AC-10 [REQ-012]: Schema introspection confirms all expected FK constraints', () => {
  test('all 5 expected FK constraints present with correct delete rules', async () => {
    ...
    // Then: all 5 expected FKs are present with correct delete rules,

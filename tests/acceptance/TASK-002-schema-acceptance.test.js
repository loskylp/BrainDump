...
describe('AC-1 [REQ-012]: All 5 tables exist after migration', () => {
  ...
  test('[VERIFIER-ADDED] exactly 6 tables exist in public schema (5 app + SequelizeMeta)', async () => {
    ...
    // Then: exactly 6 are present — 5 application tables plus SequelizeMeta
    const [rows] = await sequelize.query(
...
describe('AC-6 [REQ-011/REQ-012]: Migration role can operate without RLS blocking (OBS-002)', () => {
  ...
  test('[VERIFIER-ADDED] SequelizeMeta records all 6 migration files as applied', async () => {
    // Given: all 6 migration files have been run
    // When: SequelizeMeta is queried
    // Then: 6 entries exist — confirms no migration was skipped or failed silently

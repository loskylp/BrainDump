...
const app = require('../../backend/src/app'); // relative to project root tests/acceptance/
const { sequelize, User, Note, Folder, NoteVersion } = require('../../backend/src/models');
...
describe('AC-7 [REQ-011]: RLS is structurally active on all three protected tables', () => {
  ...
  it('[VERIFIER-ADDED] rlsContext middleware sets app.current_user_id — verified via rlsContext integration test baseline', async () => {
    ...
    const rlsContext = require('../../backend/src/middleware/rlsContext');
